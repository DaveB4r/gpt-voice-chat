...
export const aiMessage = async (msg: string): Promise<string> => {
  const completion = await openai.chat.completions.create({
    messages: [
      {
        role: "system",
        content: "You are a english tutor and your goal it's to help people improve their grammar, listening and speaking skills.",
      },
      {
        role: "user",
        content: msg
...
export const aiVoiceMsg = async (msg: string) =>  {
  ...
  const audioFile = path.resolve(`../client/public/${fileName}`);
